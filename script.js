var playervalue, croupier, playertotal;
var playertotal = 0;
var croupierTotal = 0;
var cardIsCreated;
...
const totalTxt = document.getElementById("totalValue");
const board = document.getElementById("board");
var canPlay = true;
var takenCardCount = 0;
var croupierFirst, croupierSecond;
...
function gamestart() {
    ...
    totalTxt.innerText = "You: " + playertotal;

    const c1 = drawCardFromDeck();
    const c2 = drawCardFromDeck();
    croupierHand.push(c1.id, c2.id);
    croupierFirst = c1.id;
    croupierSecond = c2.id;
    croupierTotal = calculateTotal(croupierHand);

    croupierFirstImg.src = getCardImage(croupierFirst);
    croupierTotalTxt.innerText = "Croupier: " + getCardValue(croupierFirst) + " + ?";
...
function croupierTurn() {
    croupierSecondImg.src = getCardImage(croupierSecond);
...
function takeCard() {
    if (canPlay) {
        ...
        const takenCardImg = document.createElement("img");
        board.appendChild(takenCardImg);
        console.log("çekilen kart sayısı: "+takenCardCount )
        
        if(takenCardCount>0)
        {
            takenCardImg.classList.add("cards4", "position-absolute");
        }

        else
        {
            takenCardImg.classList.add("cards3", "position-absolute");
        }
        takenCardImg.src = card.image;
        takenCardImg.id="takenCard"+takenCardCount
...
function control() {
    ...
    restart()
    console.log("TEMİZLENDİ")
}

function restart() {
    if(cardIsCreated==true)
        for (let i = 0; i < takenCardCount; i++) {
            document.getElementById("takenCard"+i).remove();
        }
...
btnStand.addEventListener("click", stand); // ✅ Event listener
